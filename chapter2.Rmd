...
```
Dataset is a summary of questionnaire results from 166 students who completed the exam, for which *gender* (M=male, F=female), *age* (17-55 years), and exam results (*points*) is known. The students with 0 points from the exam have been removed from data; for the students who attempted to finish the exam, exam results vary from 7-33 points. The questionnaire included questions measuring attitude and different learning strategies (deep, surface, or strategic learning). Corresponding summary variables *deep*, *surf* and *stra* have been transformed to equal scale by taking rowmeans (by calculating means for each of all questions measuring that specific learning strategy, separately for each student.)   
...
Boxplots will summarize data distribution nicely. However, if variables are in a very different scale, some of the plots may not be very visible. So, we first check which of the numerical variables are in the same range.  
```{r}
...
```{r}
pairs(dat[,-1],col=dat$gender) #scatterplot matrix of all numerical variables (excluding column 1, which has a categorical variable) and colors points by gender  
```   
